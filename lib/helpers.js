...
function addAmbiguousAdjective(wordData, ambiguousAdjectives) {
  const beforeAndMiddle = wordData.precedingWordTags && isPrecedingElementAWord(wordData.precedingWordTags);
...
function getBeforeAndMiddleTagSequencesData(wordData) {
  beforeWordTag = convertTag(wordData.precedingWordTags[0]);
...
function getMiddleAndAfterTagSequencesData(wordData) {
  afterWordTag = convertTag(wordData.followingWordTags[0]);
...
const mergeSameAdjectives = (cerAdjs) => {
  ...
  while (i--) {
    ...
    if (cerAdjs[i].word === cerAdjs[nextElIndex].word) {
      cerAdjs[i].frequency += cerAdjs[nextElIndex].frequency; // ramp up the contrast
